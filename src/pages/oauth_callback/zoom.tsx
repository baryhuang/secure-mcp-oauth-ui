...
export default function ZoomOAuthCallback() {
  ...
  useEffect(() => {
    ...
    if (code) {
      ...
      const exchangeCodeForToken = async () => {
        try {
          ...
          const url = `${API_BASE_URL}/api/oauth/callback/zoom?${params.toString()}`;
          ...
          if (data.success && data.user_info && data.token_info) {
            ...
            localStorage.setItem(
              `oauth_token_zoom_${userId}`,
              JSON.stringify(data.token_info)
            );
            
            // Store user info
            localStorage.setItem(
              `oauth_user_zoom_${userId}`,
              JSON.stringify(data.user_info)
            );
            
            console.log('Stored Zoom user and token data for:', userId);
            ...
            window.location.href = '/?code=' + code + '&state=zoom';
          } 
          // Fallback for older response format
          else if (data.user_id) {
            localStorage.setItem(
              `oauth_token_zoom_${data.user_id}`,
              JSON.stringify(data)
            );
          ...
          window.location.href = '/?error=token_exchange_failed&state=zoom';
        }
      };
    ...
    return () => {
      console.log('Zoom OAuth callback component unmounting');
    ...
    <Box height="100vh" width="100%">
      <Center height="100%">
        <VStack spacing={6}>
          ...
          <Text fontSize="xl">Processing Zoom authorization...</Text>
